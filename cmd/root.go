...
func init() {
	...
	// global
	rootCmd.PersistentFlags().BoolVar(&db.S3, "s3-db", false, "uploads everything to s3")
	...
	rootCmd.PersistentFlags().IntVar(&chrm.Delay, "delay", 1, "delay in seconds between navigation and screenshot")
	...
	rootCmd.PersistentFlags().StringVarP(&chrm.JsonDumpPath, "json-path", "J", "", "store path for json dumps (use . for pwd)")
	rootCmd.PersistentFlags().BoolVarP(&chrm.JsonDom, "json-dom", "", false, "store dom in json dumps")
	...
	rootCmd.PersistentFlags().StringVar(&chrm.ProxyUsername, "proxy-user", "", "username for proxy")
	rootCmd.PersistentFlags().StringVar(&chrm.ProxyPassword, "proxy-pass", "", "password for proxy")
	rootCmd.PersistentFlags().StringVarP(&options.AccessKey, "access-key", "", "", "access key")
	rootCmd.PersistentFlags().StringVarP(&options.Secret, "secret", "", "", "secret")
}
